...
1. How many tasks are waiting on MY approval, and that I can act on them with `agentklar ui --open` (Approvals tab) or `agentklar approve <id>`.
2. The native UI is the default view: `agentklar ui --open` (no extra service needed). Only mention the Vikunja board URL **if** `status` literally prints a `board: connected` line; if it says `not reachable` or no board line, do NOT give a board URL.
